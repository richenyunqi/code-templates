...
class SegmentTree {
  public:
    ...
    SegmentTree(gg len, gg* A = nullptr) :
        n(len), sumTree((len + 5) * 4), MinTree((len + 5) * 4), MaxTree((len + 5) * 4) {
        init(1, 1, n, A);  //根据输入序列A建立线段树
    }
    //将A[p]增加v
    void realUpdate(gg p, gg v) { update(1, 1, n, p, v); }
    //查询A的[left,right]区间的和、最小值、最大值
    array<gg, 3> realGetResult(gg left, gg right) { return getResult(1, 1, n, left, right); }
  ...
  private:
    ...
    array<gg, 3> getResult(gg root, gg l, gg r, gg left, gg right) {
        ...
        if (left <= mid) {
            auto res = getResult(root * 2, l, mid, left, min(mid, right));
            ans[0] += res[0];
            ans[1] = min(ans[1], res[1]);
            ans[2] = max(ans[2], res[2]);
        }
        if (right > mid) {
            auto res = getResult(root * 2 + 1, mid + 1, r, max(left, mid + 1), right);
            ans[0] += res[0];
            ans[1] = min(ans[1], res[1]);
            ans[2] = max(ans[2], res[2]);
        }
...
class SegmentTree {
  ...
  private:
    ...
    void update(gg root, gg l, gg r, gg left, gg right, gg v) {
        ...
        pushdown(root, l, r);
        if (left <= mid) {
            update(root * 2, l, mid, left, min(mid, right), v);
        }
        if (right > mid) {
            update(root * 2 + 1, mid + 1, r, max(left, mid + 1), right, v);
        }
        sumTree[root] = sumTree[root * 2] + sumTree[root * 2 + 1];
        MinTree[root] = min(MinTree[root * 2], MinTree[root * 2 + 1]);
        MaxTree[root] = max(MaxTree[root * 2], MaxTree[root * 2 + 1]);
...
class SegmentTree {
  ...
  private:
    ...
    void update(gg root, gg l, gg r, gg left, gg right, gg vmul, gg vadd) {
        ...
        pushdown(root, l, r);
        if (left <= mid) {
            update(root * 2, l, mid, left, min(mid, right), vmul, vadd);
        }
        if (right > mid) {
            update(root * 2 + 1, mid + 1, r, max(left, mid + 1), right, vmul, vadd);
        }
        sumTree[root] = (sumTree[root * 2] + sumTree[root * 2 + 1]) % Mod;
    }
    //查询A的[left,right]区间的和
    ...
    gg getResult(gg root, gg l, gg r, gg left, gg right) {
        ...
        pushdown(root, l, r);
        if (left <= mid) {
            ans += getResult(root * 2, l, mid, left, min(mid, right));
        }
        if (right > mid) {
            ans += getResult(root * 2 + 1, mid + 1, r, max(left, mid + 1), right);
        }
        return ans % Mod;
    }
};
