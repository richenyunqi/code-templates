...
```cpp
template <typename T, typename Update_Operation = function<T(T, T)>, typename Union_Operation = function<T(T, T)>>
class SegmentTree {
  public:
    struct TreeNode {
        T val;
    ...
    SegmentTree(gg len, const T& _default_value, T* A = nullptr, const Update_Operation& _update_op = plus<T>(),
        ...
        n(len),
        default_value(_default_value), update_op(_update_op), union_op(_union_op) {
        root = init(1, n, A);
    }
    // update_op(A[p],v)
...
```cpp
template <typename T, typename Update_Operation = function<T(T, T)>, typename Union_Operation = function<T(T, T)>,
          typename Lazy_Operation = multiplies<T>, typename Lazy_Union_Operation = function<T(T, T)>>
class SegmentTree {
  public:
    struct TreeNode {
        T val, lazy;
...
template <typename T, typename Update_Operation = function<T(T, T, T)>, typename Union_Operation = function<T(T, T)>,
          typename Lazy_Add = function<T(T, T)>, typename Lazy_Union_Add = function<T(T, T)>, typename Lazy_Union_Mul = function<T(T, T)>>
class SegmentTree {
  public:
    struct TreeNode {
