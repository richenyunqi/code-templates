...
public:
    ...
    SegmentTree(gg len, const T &_default_value, T *A = nullptr, const Update_Operation &_update_op = plus<T>(),
                const Union_Operation &_union_op = plus<T>()) :
        n(len), default_value(_default_value), update_op(_update_op), union_op(_union_op) {
        root = init(1, n, A);
    }
    // update_op(A[p],v)
    void realUpdate(gg p, const T &v) {
        update(root, 1, n, p, v);
    }
    //查询A的[left,right]区间执行union_op后的值
    T realGetResult(gg left, gg right) {
        return getResult(root, 1, n, left, right);
    }
...
private:
    ...
    TreeNode *init(gg left, gg right, T *A) {
        ...
        TreeNode *root = A ? new TreeNode(A[left]) : new TreeNode(default_value);
        gg mid = (left + right) / 2;
...
class SegmentTree {
  public:
    ...
    SegmentTree(gg len, const T& _sequence_default_value, const T& _lazy_default_value, T* A = nullptr,
                const Update_Operation& _update_op = plus<T>(), const Union_Operation& _union_op = plus<T>(),
                const Lazy_Operation& _lazy_op = multiplies<T>(),
                const Lazy_Union_Operation& _lazy_union_op = plus<T>()) :
        n(len),
        sequence_default_value(_sequence_default_value), lazy_default_value(_lazy_default_value), update_op(_update_op),
        union_op(_union_op), lazy_op(_lazy_op), lazy_union_op(_lazy_union_op) {
        root = init(1, n, A);
    }
  ...
  private:
    ...
    TreeNode* init(gg left, gg right, T* A) {
        ...
        TreeNode* root =
            A ? new TreeNode(A[left], lazy_default_value) : new TreeNode(sequence_default_value, lazy_default_value);
        gg mid = (left + right) / 2;
        root->left = init(left, mid, A);
        root->right = init(mid + 1, right, A);
        root->val = union_op(root->left->val, root->right->val);
...
class SegmentTree {
  public:
    ...
    SegmentTree(gg len, const T& _sequence_default_value, const T& _lazy_mul_value, const T& _lazy_add_value,
                T* A = nullptr, const Update_Operation& _update_op = MulAdd<T>(),
                const Union_Operation& _union_op = Add<T>(), const Lazy_Add& _lazy_add = Mul<T>(),
                const Lazy_Union_Add& _lazy_union_add = MulAdd<T>(), const Lazy_Union_Mul& _lazy_union_mul = Mul<T>()) :
        n(len),
        sequence_default_value(_sequence_default_value), lazy_add_value(_lazy_add_value),
        lazy_mul_value(_lazy_mul_value), update_op(_update_op), union_op(_union_op), lazy_add(_lazy_add),
        lazy_union_add(_lazy_union_add), lazy_union_mul(_lazy_union_mul) {
        root = init(1, n, A);
    }
  ...
  private:
    ...
    TreeNode* init(gg left, gg right, T* A) {
        ...
        TreeNode* root = A ? new TreeNode(A[left], lazy_mul_value, lazy_add_value) :
                             new TreeNode(sequence_default_value, lazy_mul_value, lazy_add_value);
        gg mid = (left + right) / 2;
        root->left = init(left, mid, A);
        root->right = init(mid + 1, right, A);
